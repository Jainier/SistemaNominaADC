...
public class TipoPermisoService : ITipoPermisoService
{
    ...
    public async Task<bool> Actualizar(TipoPermiso modelo) { await Validar(modelo, modelo.IdTipoPermiso); var a = await _context.TipoPermisos.FirstOrDefaultAsync(x => x.IdTipoPermiso == modelo.IdTipoPermiso) ?? throw new NotFoundException("Tipo de permiso no encontrado."); a.Nombre = modelo.Nombre; a.ConGoceSalarial = modelo.ConGoceSalarial; a.IdEstado = modelo.IdEstado; return await _context.SaveChangesAsync() > 0; }
    public async Task<bool> Desactivar(int id) { var a = await _context.TipoPermisos.FirstOrDefaultAsync(x => x.IdTipoPermiso == id) ?? throw new NotFoundException("Tipo de permiso no encontrado."); a.IdEstado = await EstadoSistemaHelper.ObtenerIdEstadoInactivoAsync(_context); return await _context.SaveChangesAsync() > 0; }
