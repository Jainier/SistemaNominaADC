...
public class TipoHoraExtraService : ITipoHoraExtraService
{
    ...
    public async Task<bool> Desactivar(int id) { var a = await _context.TipoHoraExtras.FirstOrDefaultAsync(x => x.IdTipoHoraExtra == id) ?? throw new NotFoundException("Tipo de hora extra no encontrado."); a.IdEstado = await EstadoSistemaHelper.ObtenerIdEstadoInactivoAsync(_context); return await _context.SaveChangesAsync() > 0; }
    private async Task Validar(TipoHoraExtra modelo, int id)
    {
        ...
        if (modelo.PorcentajePago.Value < 1m || modelo.PorcentajePago.Value > 9.9999m) throw new BusinessException("El porcentaje de pago debe ser mayor o igual a 1 y menor o igual a 9.9999.");
