...
public class TipoIncapacidadService : ITipoIncapacidadService
{
    ...
    public async Task<bool> Desactivar(int id) { var a = await _context.TipoIncapacidades.FirstOrDefaultAsync(x => x.IdTipoIncapacidad == id) ?? throw new NotFoundException("Tipo de incapacidad no encontrado."); a.IdEstado = await EstadoSistemaHelper.ObtenerIdEstadoInactivoAsync(_context); return await _context.SaveChangesAsync() > 0; }
