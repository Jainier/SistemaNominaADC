...
namespace SistemaNominaADC.Api.Controllers
{
    ...
    public class ObjetosSistemaController : ControllerBase
    {
        ...
        [HttpPost("Guardar")]
        public async Task<IActionResult> Guardar([FromBody] ObjetoSistema entidad) => Ok(await _objetoService.Guardar(entidad));

        [HttpGet("Obtener/{nombre}")]
        public async Task<IActionResult> Obtener(string nombre) => Ok(await _objetoService.ObtenerPorNombre(nombre));
