...
public class TipoHoraExtraController : ControllerBase
{
    ...
    public TipoHoraExtraController(ITipoHoraExtraService service, IObjetoSistemaAuthorizationService objetoAuthService)
    {
        _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Crear([FromBody] TipoHoraExtra dto)
    {
        ...
        return CreatedAtAction(nameof(Obtener), new { id = creado.IdTipoHoraExtra }, creado);
    }

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Actualizar(int id, [FromBody] TipoHoraExtra dto)
    {
        var acceso = await ValidarAccesoModuloAsync();
        if (acceso != null) return acceso;

        if (id != dto.IdTipoHoraExtra) return ValidationProblem(new ValidationProblemDetails(new Dictionary<string, string[]> { ["id"] = ["El id no coincide con el cuerpo"] }));
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
    ...
    {
        var autorizado = await _objetoAuthService.PuedeAccederModuloAsync(User, "TipoHoraExtra");
        return autorizado ? null : Forbid();
    }

    private async Task<IActionResult?> ValidarConsultaCatalogoAsync()
    {
        var autorizado = await _objetoAuthService.PuedeConsultarCatalogoAsync(User, "TipoHoraExtra");
        return autorizado ? null : Forbid();
    }
}
