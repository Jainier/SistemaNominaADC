...
[ApiController]
[Authorize]
public class TipoConceptoNominaController : ControllerBase
{
    private readonly ITipoConceptoNominaService _service;
    private readonly IObjetoSistemaAuthorizationService _objetoAuthService;

    public TipoConceptoNominaController(
        ITipoConceptoNominaService service,
    ...
    [HttpPost]
    public async Task<IActionResult> Crear([FromBody] TipoConceptoNomina dto)
    {
        ...
        return CreatedAtAction(nameof(Obtener), new { id = creado.IdConceptoNomina }, creado);
    }

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Actualizar(int id, [FromBody] TipoConceptoNomina dto)
    {
        var acceso = await ValidarAccesoModuloAsync();
        if (acceso != null) return acceso;

        if (id != dto.IdConceptoNomina) return ValidationProblem(new ValidationProblemDetails(new Dictionary<string, string[]> { ["id"] = ["El id no coincide con el cuerpo"] }));
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
    ...
    {
        var autorizado = await _objetoAuthService.PuedeAccederModuloAsync(User, "TipoConceptoNomina");
        return autorizado ? null : Forbid();
    }

    private async Task<IActionResult?> ValidarConsultaCatalogoAsync()
    {
        var autorizado = await _objetoAuthService.PuedeConsultarCatalogoAsync(User, "TipoConceptoNomina");
        return autorizado ? null : Forbid();
    }
}
